...
private extension NetworkLoggerPlugin {
  ...
  func logNetworkRequest(_ request: URLRequest?) -> [String] {
    ...
    if let body = request?.httpBody,
       let stringOutput = requestDataFormatter?(body) ?? String(data: body,
                                                                encoding: .utf8),
       isVerbose {
      output += [format(loggerId, date: date,
  ...
  func logNetworkResponse(_ response: HTTPURLResponse?,
                          data: Data?,
                          target: TargetType) -> [String] {
    guard let response = response else {
      return [format(loggerId, date: date,
                     identifier: "Response",
    ...
    if let data = data,
       let stringData = String(data: responseDataFormatter?(data) ?? data,
